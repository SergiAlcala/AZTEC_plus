...
def load_scalers(city,save_folder, ppf_static, ppf_helper):
    static_scaler = pickle.load(open(os.path.join(ROOT_DIR,f'{save_folder}/{city}/PHI_{phi}/Th_{delay}/block_1_minmaxscaler.pkl'), 'rb'))
    shared_scaler = pickle.load(open(os.path.join(ROOT_DIR,f'{save_folder}/{city}/PHI_{phi}/Block_2_3_results/ppf_static_{ppf_static}_ppf_helper_{ppf_helper}/Th_{delay}/block_2_minmaxscaler.pkl'), 'rb'))
        ...
        for ppf_static in tqdm(ppf_static_list):
            for phi in tqdm(PHIS):  
                for alpha in tqdm(ALPHAS):
                    delay=np.load(f'{ROOT_DIR}/{save_folder}/{city}/PHI_{phi}/overall_optimal_window.npy')
                    # delay=5
                    ...
                    allocation_loop(static_norm, shared_norm, mae_norm, output_norm, num_services,  phi, alpha,Filesave)
