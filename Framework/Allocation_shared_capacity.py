...
def upload_static_shared(phi, alpha, delay, num_services, B):
    cap_static = np.load('Results/cap_static/Th_%s/capacity_forecasted_uncer_test_2_slopes_more_services_delay_%s_phi_%s_gamma_2_deltax_005.npy' % (delay, delay, phi))
    cap_shared = np.load('Results/cap_shared/Th_%s/shared_upper_forecasted_more_services_delay_%s_phi_%s_gamma_2_alpha_%s_ppf_static_075_ppf_mae_095_dropout_05_03.npy' % (delay, delay, phi, alpha))
...
def main(phi, alpha):
    ...
    delay = 12
    B = 100
    youtube = np.load('../journal_extension/youtube/time_load_matrix.npy')
    snapchat = np.load('../journal_extension/snapchat/time_load_matrix.npy')
    facebook = np.load('../journal_extension/facebook/time_load_matrix.npy')
    itunes = np.load('../journal_extension/itunes/time_load_matrix.npy')
    instagram = np.load('../journal_extension/instagram/time_load_matrix.npy')
    ...
    agg_output = np.stack((agg_output_youtube[:-1], agg_output_snapchat,
                           agg_output_facebook[:-1], agg_output_itunes[:-1],
                           agg_output_instagram), axis=-1)
    max_concatenated = np.stack((max_youtube, max_snapchat,
                                 max_facebook, max_itunes,
                                 max_instagram))
    mae_forecasting = np.load('Results/mae_merged_dropout_05_03_test.npy')

    static, shared = upload_static_shared(phi, alpha, delay, num_services, B)

    max_shared = np.load('Results/cap_shared/Th_%s/max_upper_shared_delay_%s_phi_%s_alpha_%s_mae_input_ppf_static_075_ppf_mae_095.npy' % (delay, delay, phi, alpha))
    ...
    for time in range(0, 2010):
        ...
        if time == idx_time * 200:
            ...
            np.save('cplus_phi_%s_alpha_%s_delay_%s_ppf_static_075_ppf_mae_095.npy' % (phi, alpha, delay), cplus)
            np.save('slas_phi_%s_alpha_%s_delay_%s_ppf_static_075_ppf_mae_095.npy' % (phi, alpha, delay), slas)
            idx_time += 1
    np.save('cplus_phi_%s_alpha_%s_delay_%s_ppf_static_075_ppf_mae_095.npy' % (phi, alpha, delay), cplus)
    np.save('slas_phi_%s_alpha_%s_delay_%s_ppf_static_075_ppf_mae_095.npy' % (phi, alpha, delay), slas)
    return 0
